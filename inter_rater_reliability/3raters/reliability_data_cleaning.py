...
def clean_out_not_qualified_data(download_path, units):
    ...
    with open(f"{download_path}/rater_not_equal_to_3.json", "w") as f:
...
path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "extract_data", f"3raters/{folder_name}/valid_data.json"))
data  = load_file(path)
