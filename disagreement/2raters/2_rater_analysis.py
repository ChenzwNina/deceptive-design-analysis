...
def analyze_annotations(path):
    ...
    for widget_id, annots in widget_annotations.items():
        ...
        for cat, raters in category_votes.items():
            # Case A: exactly one annotator missed this category (1 of 2 selected)
            if len(raters) == 1 and len(annots) == 2:
...
if __name__ == "__main__":
    ...
    path = os.path.abspath(os.path.join(
        os.path.dirname(__file__), "..", "..", f"extract_data/2raters/round{round_number}/leica-nina/valid_data.json"
    ))
    ...
    #  missed_by_annotator,
    # missed_by_one_widgets,
     selected_by_one_widgets) = analyze_annotations(path)

    all_cats = sorted(set(selected_by_one))
    for cat in all_cats:
